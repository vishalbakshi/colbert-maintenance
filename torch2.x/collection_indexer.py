...
class CollectionIndexer():
    ...
    def setup(self):
        ...
        torch.save(sampled_pids, f"{ROOT}/sample_pids.pt") # ADDED BY VISHAL
        avg_doclen_est = self._sample_embeddings(sampled_pids)

        # Select the number of partitions
        num_passages = len(self.collection)
        torch.save(num_passages, f"{ROOT}/num_passages.pt") # ADDED BY VISHAL
    ...
    def train(self, shared_lists):
        ...
        torch.save(centroids, f"{ROOT}/centroids.pt") # ADDED BY VISHAL

        print_memory_stats(f'RANK:{self.rank}')
        del sample

        bucket_cutoffs, bucket_weights, avg_residual = self._compute_avg_residual(centroids, heldout)
        torch.save(bucket_cutoffs, f"{ROOT}/bucket_cutoffs.pt") # ADDED BY VISHAL
        torch.save(bucket_weights, f"{ROOT}/bucket_weights.pt") # ADDED BY VISHAL
        torch.save(avg_residual,   f"{ROOT}/avg_residual.pt") # ADDED BY VISHAL
    ...
    def _concatenate_and_split_sample(self):
        ...
        torch.save(sample, f"{ROOT}/sample.pt") # ADDED BY VISHAL
        torch.save(sample_heldout, f"{ROOT}/sample_heldout.pt") # ADDED BY VISHAL
    ...
    def _train_kmeans(self, sample, shared_lists):
        ...
        if do_fork_for_faiss:
            ...
            args_ = args_ + [[[sample]]]
            torch.save(args_, f"{ROOT}/args_.pt") # ADDED BY VISHAL
    ...
    def _build_ivf(self):
        ...
        if self.verbose > 1:
            ...
            print_memory_stats(f'RANK:{self.rank}')
        torch.save(codes, f"{ROOT}/presort_codes.pt") # ADDED BY VISHAL
        codes = codes.sort()
        ivf, values = codes.indices, codes.values
        
        torch.save(codes, f"{ROOT}/codes.pt") # ADDED BY VISHAL
        torch.save(ivf, f"{ROOT}/ivf.pt") # ADDED BY VISHAL
        torch.save(values, f"{ROOT}/values.pt") # ADDED BY VISHAL
